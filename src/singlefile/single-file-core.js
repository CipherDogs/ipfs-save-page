  ...
  (() => {
    ...
    const STAGES = [
      {
        ...
        parallel: [{ action: 'resolveStylesheetURLs' }, { option: '!removeFrames', action: 'resolveFrameURLs' }, { action: 'resolveHtmlImportURLs' }],
      },
      {
        sequential: [
    ...
    class Processor {
      ...
      async getPageData() {
        ...
        filename = filename.replace(/[~\\?%*:|"<>\x00-\x1f\x7F]+/g, replacementCharacter); // eslint-disable-line no-control-regex
        filename = filename
          .replace(/\.\.\//g, '')
          .replace(/^\/+/, '')
          .replace(/\/+/g, '/')
      ...
      preProcessPage() {
        if (this.options.win) {
          this.doc.body
            ...
            .forEach(element => element instanceof this.options.win.HTMLElement && this.doc.head.appendChild(element));
        }
      ...
      removeDiscardedResources() {
        ...
        const objectElements = this.doc.querySelectorAll(
          'applet, object[data]:not([type="image/svg+xml"]):not([type="image/svg-xml"]):not([type="text/html"]), embed[src]:not([src*=".svg"]):not([src*=".pdf"])'
        );
        ...
        replacedAttributeValue.forEach(element => {
          const relValue = element
            .getAttribute('rel')
            .replace(/(preconnect|prerender|dns-prefetch|preload|prefetch)/g, '')
            .trim();
          if (relValue.length) {
      ...
      async processStylesheets() {
        await Promise.all(
          Array.from(this.stylesheets, ([, stylesheetInfo]) =>
      ...
      async processStyleAttributes() {
        return Promise.all(
          Array.from(this.styles, ([, declarationList]) =>
      ...
      async processPageResources() {
        ...
        let resourcePromises = processAttributeArgs.map(([selector, attributeName, processDuplicates, removeElementIfMissing]) =>
          ProcessorHelper.processAttribute(
      ...
      async processScripts() {
        await Promise.all(
          Array.from(this.doc.querySelectorAll('script[src]'), async scriptElement => {
            let resourceURL;
    ...
    class ProcessorHelper {
      static async evalTemplate(template = '', options, content, dontReplaceSlash) {
        ...
        template = await Util.evalTemplateVariable(
          template,
          'page-description',
          () => options.info.description || 'No description',
          dontReplaceSlash,
        ...
        template = await Util.evalTemplateVariable(
          template,
          'page-publisher',
          () => options.info.publisher || 'No publisher',
          dontReplaceSlash,
          options.filenameReplacementCharacter
        );
        ...
        template = await Util.evalTemplateVariable(
          template,
          'url-hostname',
          () => url.hostname.replace(/\/$/, '') || 'No hostname',
          dontReplaceSlash,
          options.filenameReplacementCharacter
        );
        ...
        template = await Util.evalTemplateVariable(
          template,
          'url-referrer',
          () => decodeURI(options.referrer) || 'No referrer',
          dontReplaceSlash,
          options.filenameReplacementCharacter
        );
        ...
        template = await Util.evalTemplateVariable(
          ...
          () =>
            decodeURI(url.pathname)
              .replace(/^\//, '')
              .replace(/\/$/, '') || 'No pathname',
          dontReplaceSlash === undefined ? true : dontReplaceSlash,
          options.filenameReplacementCharacter
        );
        ...
        template = await Util.evalTemplateVariable(
          template,
          'url-last-segment',
          () => decodeURI(Util.getLastSegment(url, options.filenameReplacementCharacter)) || 'No last segment',
          dontReplaceSlash,
          options.filenameReplacementCharacter
        );
        if (content) {
          template = await Util.evalTemplateVariable(
            template,
            'digest-sha-256',
            async () => util.digest('SHA-256', content),
            dontReplaceSlash,
            options.filenameReplacementCharacter
          );
          template = await Util.evalTemplateVariable(
            template,
            'digest-sha-384',
            async () => util.digest('SHA-384', content),
            dontReplaceSlash,
            options.filenameReplacementCharacter
          );
          template = await Util.evalTemplateVariable(
            template,
            'digest-sha-512',
            async () => util.digest('SHA-512', content),
            dontReplaceSlash,
            options.filenameReplacementCharacter
          );
        }
          ...
          if (!Util.testIgnoredPath(resourceURL)) {
            if (!resourceURL || Util.testValidPath(resourceURL)) {
              ...
              if (Util.testValidURL(resolvedURL) && resourceURL != resolvedURL && stylesheetContent.includes(urlFunction)) {
                stylesheetContent = stylesheetContent.replace(
                  Util.getRegExp(urlFunction),
                  originalResourceURL ? urlFunction.replace(originalResourceURL, resolvedURL) : 'url(' + resolvedURL + ')'
                );
              }
            } else {
              let newUrlFunction;
      ...
      static async processStylesheet(cssRules, baseURI, options, cssVariables, batchRequest) {
        ...
        async function processFontFaceRule(ruleData) {
          await Promise.all(
            ruleData.block.children.toArray().map(async declaration => {
              if (declaration.type == 'Declaration' && declaration.value.children) {
                ...
                await Promise.all(
                  urlFunctions.map(async urlFunction => {
                    const originalResourceURL = Util.matchURL(urlFunction);
                    const resourceURL = Util.normalizeURL(originalResourceURL);
                    if (!Util.testIgnoredPath(resourceURL)) {
                      if (Util.testValidURL(resourceURL)) {
      ...
      static async processAttribute(resourceElements, attributeName, baseURI, options, cssVariables, styles, batchRequest, processDuplicates, removeElementIfMissing) {
        await Promise.all(
          Array.from(resourceElements, async resourceElement => {
        ...
        if (attributeValue) {
          ...
          if (imageData && imageData.replaceable) {
            imgElement.setAttribute(
              'src',
              `${PREFIX_DATA_URI_IMAGE_SVG},<svg xmlns="http://www.w3.org/2000/svg" width="${imageData.size.pxWidth}" height="${imageData.size.pxHeight}"><rect fill-opacity="0"/></svg>`
            );
    ...
    class Util {
      ...
      static matchURL(stylesheetContent) {
        const match =
          stylesheetContent.match(REGEXP_URL_SIMPLE_QUOTES_FN) || stylesheetContent.match(REGEXP_URL_DOUBLE_QUOTES_FN) || stylesheetContent.match(REGEXP_URL_NO_QUOTES_FN);
      ...
      static testValidURL(resourceURL) {
        return (
          Util.testValidPath(resourceURL) &&
          (resourceURL.match(HTTP_URI_PREFIX) || resourceURL.match(FILE_URI_PREFIX) || resourceURL.startsWith(BLOB_URI_PREFIX)) &&
          resourceURL.match(NOT_EMPTY_URL)
        );
      }

      static matchImport(stylesheetContent) {
        const match =
          stylesheetContent.match(REGEXP_IMPORT_URL_SIMPLE_QUOTES_FN) ||
